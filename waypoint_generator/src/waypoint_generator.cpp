...
std_msgs::Bool waypoints_receive_lable;

nav_msgs::Odometry odom;
nav_msgs::Path waypoints;
geometry_msgs::PoseStamped pt_old;
...
void goal_callback(const geometry_msgs::PoseStamped::ConstPtr& msg)
{
    ...
    waypoints_receive_lable.data = true;
    ...
    else
    {
        if (msg->pose.position.z > 0)
        {
            ...
            if (fabs(pt_old.pose.position.x - pt.pose.position.x) > 1e-3 ||
                fabs(pt_old.pose.position.y - pt.pose.position.y) > 1e-3 ||
                fabs(pt_old.pose.position.z - pt.pose.position.z) > 1e-3)
            {
                waypoints.poses.push_back(pt);
                publish_waypoints_vis();
                pt_old = pt;
    ...
    pub3.publish(waypoints_receive_lable);
    waypoints_receive_lable.data = false;
}
...
int main(int argc, char** argv)
{
    ...
    pub3 = n.advertise<std_msgs::Bool>("waypoints_receive_lable", 10);
